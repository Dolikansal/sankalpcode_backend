...
const submittoken = async (tokens) => {
  ...
  let attempts = 0;
  const maxAttempts = 10; // Max 10 baar check karega (Total ~20 seconds)

  while (attempts < maxAttempts) {
    try {
      console.log(`🔍 Polling Judge0 (Attempt ${attempts + 1}/${maxAttempts})...`);
      const response = await axios.request(options);
      ...
      // Agar API key/quota error (401, 429) aaye toh turant break karein
      if (error.response?.status === 429 || error.response?.status === 401) {
        throw new Error(`Judge0 API Error: ${error.response.statusText}`);
      }
    }

    attempts++;
    await waiting(2000); // 2 sec delay
  }
